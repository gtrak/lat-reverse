...
3. Launch a `Task` subagent (type from `split.md`) to survey the given scope. Include the workflow contents in its prompt. Tell it: "Return a structured summary of this scope: files, their responsibilities, and how they relate." For large scopes, launch multiple explore subagents scoped to different directories.
